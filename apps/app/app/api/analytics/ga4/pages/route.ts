/**
 * GET /api/analytics/ga4/pages?clientId=&range=
 *
 * Returns the top 10 landing pages with sessions + conversions.
 */
...
import { fetchGa4Pages } from "@/lib/analytics/ga4-api";

export const dynamic = "force-dynamic";

export async function GET(request: NextRequest) {
  if (!(await requireAutomationAuth(request, "GET /api/analytics/ga4/pages", ApiScope.ANALYTICS_READ))) {
  ...
  try {
    const pages = await fetchGa4Pages(cred.accountId, cred.refreshToken, range);
